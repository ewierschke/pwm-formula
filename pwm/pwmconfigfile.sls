...
aws s3 cp s3://{{ salt['environ.get']('CONFIGBUCKETNAME') }}/18branch/PwmConfiguration.xml /usr/share/tomcat/webapps/ROOT/WEB-INF/PwmConfiguration.xml:
  cmd.run
...
/usr/local/bin/pwmconfmgmt.sh:
  file.append:
    - text: |
        ...
        aws s3 cp /usr/share/tomcat/webapps/ROOT/WEB-INF/PwmConfiguration.xml s3://$configbucketname/18branch/PwmConfiguration.xml
        logger "s3 put conf.xml file"
        aws s3 cp /tmp/PwmConfiguration.xml.sha1 s3://$configbucketname/18branch/PwmConfiguration.xml.sha1 --acl public-read
