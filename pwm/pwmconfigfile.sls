...
s3cmd get s3://{{ salt['environ.get']('CONFIGBUCKETNAME') }}/18branch/PwmConfiguration.xml /usr/share/tomcat/webapps/pwm/WEB-INF/PwmConfiguration.xml --skip-existing:
  cmd.run
...
/usr/local/bin/pwmconfmgmt:
  file.append:
    - text: |
        ...
        s3cmd put /usr/share/tomcat/webapps/pwm/WEB-INF/PwmConfiguration.xml s3://$configbucketname/18branch/PwmConfiguration.xml
        logger "s3 put conf.xml file"
        s3cmd put -P /tmp/PwmConfiguration.xml.md5 s3://$configbucketname/18branch/PwmConfiguration.xml.md5
